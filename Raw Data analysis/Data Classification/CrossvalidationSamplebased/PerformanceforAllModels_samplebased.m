...
trainingObs = obs(holdoutCVP.training,:);
trainingClassLabel = grp(holdoutCVP.training);

%Validation/TestingSet
validationObs = obs(holdoutCVP.test,:);
validationClassLabel = grp(holdoutCVP.test);
...
classifiers = {'Fine knn', 'Weighted knn', 'LDA', 'SVM' , 'DiagQuadratic' };
...
% x=1:5;
% y = [CVclassErrortrain ];
% std_dev = [CVclassError_stdtrain];
% 
% figure()
% hold on
% bar(x,y)
% errorbar(y,std_dev ,'.');
% title('CV Train Class Error');
% 
% XTickLabel= classifiers;
% XTick = [1:5];
% set(gca, 'XTick',XTick);
