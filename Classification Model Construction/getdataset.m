...
for channel = 1:length(channels)
    ...
    psdLRL_channel = dataset.outputdata.psd.LRL.signal(:,:,:,channel);
    beforeeventMATchannel = [];
    aftereventMATchannel = [];
    classelabel_beforeeventMAT =[];
    classelabel_aftereventMAT =[];
    % TRIAL NUMBERdata
    for event = 1:size(dataset.outputdata.psd.LRL.signal,3)
        
        psdLRL_channel_event = dataset.outputdata.psd.LRL.signal(:,:,event,1);
